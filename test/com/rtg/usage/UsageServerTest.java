...
public class UsageServerTest extends TestCase {
  ...
  private UUID runUsageTest1(TestDirectory dir) throws IOException {
    ...
    us.setStopTimer(1);
    us.start();
    final HttpUsageTrackingClient http = new HttpUsageTrackingClient("http://localhost:" + us.getPort() + "/", UsageConfigurationTest.createSimpleConfiguration(null, null, null, null, null), false);
  ...
  public void testWithOptionalFields() throws IOException {
    try (TestDirectory dir = new TestDirectory()) {
      ...
      final HttpUsageTrackingClient http = new HttpUsageTrackingClient("http://localhost:" + us.getPort() + "/usage", UsageConfigurationTest.createSimpleConfiguration(null, null, true, true, true), false);
      final UUID runId = UUID.randomUUID();
      http.recordBeginning("testModule", runId);
      http.recordEnd(65, "testModule", runId, true);
      stupidWorkaround(http);
  ...
  public void testErrors() throws Exception {
    ...
    try {
      try (TestDirectory dir = new TestDirectory()) {
        ...
        stupidWorkaround(new HttpUsageTrackingClient("http://localhost:" + us.getPort() + "/usage", UsageConfigurationTest.createSimpleConfiguration(null, null, null, null, null), false));
        us.end();
        final File[] files = dir.listFiles();
        assertNotNull(files);
        assertEquals(1, files.length);
  ...
  public void testDateRollover() throws Exception {
    try (TestDirectory dir = new TestDirectory()) {
      ...
      us.start();
      final HttpUsageTrackingClient http = new HttpUsageTrackingClient("http://localhost:" + us.getPort() + "/usage", new UsageConfiguration(new Properties()), true);
      final UUID runId = UUID.randomUUID();
      http.recordBeginning("testModule", runId);
      http.recordEnd(65, "testModule", runId, true);
      stupidWorkaround(http);
  ...
  private void stupidWorkaround(final HttpUsageTrackingClient client) {
