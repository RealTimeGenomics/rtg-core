...
/**
 */
public class FileUsageTrackingClientTest extends TestCase {
  ...
  public void test() throws Exception {
    try (TestDirectory dir = new TestDirectory()) {
      ...
      try (final RandomAccessFile raf = getRandomAccessFile(dir, testData)) {
        final String result = FileUsageTrackingClient.getLastKey(raf);
  ...
  public void test3Extreme() throws Exception {
    try (TestDirectory dir = new TestDirectory()) {
      ...
      try (final RandomAccessFile raf = getRandomAccessFile(dir, testData)) {
        for (int i = 1; i < 11; i++) {
          final String result = FileUsageTrackingClient.getLastKey(raf, i);
          assertEquals("xxxxx1234gogog", result);
        }
      }
    }
  ...
  public void testMessageWriting() throws Exception {
    try (TestDirectory dir = new TestDirectory()) {
      final FileUsageTrackingClient client = new FileUsageTrackingClient(dir, UsageConfigurationTest.createSimpleConfiguration(null, null, null, null, null), false);
  ...
  public void testMessageWritingOptionalFields() throws Exception {
    try (TestDirectory dir = new TestDirectory()) {
      ...
      final FileUsageTrackingClient client = new FileUsageTrackingClient(dir, UsageConfigurationTest.createSimpleConfiguration(null, null, true, true, true), false);
      final UUID runId = UUID.randomUUID();
      client.recordBeginning("unitTest", runId);
      client.recordEnd(42, "unitTest", runId, true);
      final File[] files = dir.listFiles();
  ...
  public void testMessageWritingWithCorruption() throws Exception {
    try (TestDirectory dir = new TestDirectory()) {
      final FileUsageTrackingClient client = new FileUsageTrackingClient(dir, new UsageConfiguration(new Properties()), false);
      final UUID runId = UUID.randomUUID();
  ...
  public void testPermissions2() throws IOException {
    try (TestDirectory dir = new TestDirectory()) {
      if (dir.setWritable(false, false)) { // may not work on Windows machines?
        final FileUsageTrackingClient client = new FileUsageTrackingClient(dir, new UsageConfiguration(new Properties()), true);
        final UUID runId = UUID.randomUUID();
        try {
          client.recordBeginning("unitTest", runId);
