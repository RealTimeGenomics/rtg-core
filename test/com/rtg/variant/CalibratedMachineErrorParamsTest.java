...
public class CalibratedMachineErrorParamsTest extends TestCase {
  ...
  public void testCurve() throws IOException {
    ...
    try {
      ...
      assertEquals((int) TEST_CALIBRATION_QUALITIES.get(2), cp.getPhred((char) ('!' + 2), 1, Arm.LEFT));
      assertEquals((int) TEST_CALIBRATION_QUALITIES.get(20), cp.getPhred((char) ('!' + 20), 1, Arm.LEFT));
      assertEquals((int) TEST_CALIBRATION_QUALITIES.get(30), cp.getPhred((char) ('!' + 30), 1, Arm.LEFT));
      assertEquals((int) TEST_CALIBRATION_QUALITIES.get(32), cp.getPhred((char) ('!' + 32), 1, Arm.LEFT));
      assertEquals((int) TEST_CALIBRATION_QUALITIES.get(36), cp.getPhred((char) ('!' + 36), 1, Arm.LEFT));
    } finally {
  ...
  public void testCurve2() throws IOException {
    ...
    try {
      ...
      c.accumulate(cal);
      final CalibratedMachineErrorParams cp = new CalibratedMachineErrorParams(MachineType.ILLUMINA_PE, c, "readgroup1");
      assertEquals(3, cp.getPhred((char) ('!' + 2), 1, Arm.LEFT));
    } finally {
      assertTrue(FileHelper.deleteAll(dir));
    }
  }
  ...
  public void testCurveLimit() throws IOException {
    ...
    try {
      ...
      assertEquals((int) TEST_CALIBRATION_QUALITIES.get(2), cp.getPhred((char) ('!' + 2), 1, Arm.LEFT));
      assertEquals((int) TEST_CALIBRATION_QUALITIES.get(36), cp.getPhred((char) ('!' + 36), 1, Arm.LEFT));
      assertEquals((int) TEST_CALIBRATION_QUALITIES.get(63), cp.getPhred((char) ('!' + 63), 1, Arm.LEFT));
      // test that we return the max when asking for higher values
      assertEquals((int) TEST_CALIBRATION_QUALITIES.get(63), cp.getPhred((char) ('!' + 64), 1, Arm.LEFT));
      assertEquals((int) TEST_CALIBRATION_QUALITIES.get(63), cp.getPhred((char) ('!' + 128), 1, Arm.LEFT));
