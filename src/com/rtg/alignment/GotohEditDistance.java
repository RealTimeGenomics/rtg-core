...
//  /** Discard any alignment with more than x gap opens */
//  private static final int MAX_GAPOPENS_PER_ALIGNMENT = Integer.parseInt(System.getProperty("max-gapopens-per-read", "" + Integer.MAX_VALUE));
...
//  private int mGapOpens;  //the number of gap opens we have encountered for this alignment
   ...
   */
  public GotohEditDistance(NgsParams ngsParams) {
    this(ngsParams.gapOpenPenalty(), ngsParams.gapExtendPenalty(), ngsParams.substitutionPenalty(), ngsParams.unknownsPenalty(), false);
  }
...
//      final int thisColumn = refPos * mRows; // hoist this multiplication out of the inner loop
    ...
    //System.out.println("rlen= " + rlen + " tlen=" + template.length + " rpos=" + mMinScoreReadPos + " tpos=" + mMinScoreTemplatePos);
  ...
  private int findBestReadEnd(int maxShift, int rlen, int grayWidth, int bLength, int terminationColumn) {
    ...
    for (int i = startPos; i >= refDiagEndPos - maxShift - grayWidth && i >= 0; i--) {
      ...
      //System.err.println("bot score @ (t,r)=(" + i + "," + rlen + ")=" + getInsert(bottomScores) + "," + getDistance(bottomScores) + "," + getDelete(bottomScores));
    }
...
//    System.err.println("evaluating read: " + readPos + " template: " + templatePos + " cellScore: " + (score - ZERO_SCORE) + " minScore: " + mMinScore);
  ...
  private boolean goBackwards(byte[] read, byte[] template, int readEndPos, int templateEndPos, int maxShift) {
    ...
    while (readPos > 0 || (mFixedStart && refPos != mTemplatePositionOffset)) {        //TODO uh?? why stop here in fixed start case? - mTPO is always 0 in this case
      final int command;
      //System.out.print("(" + readPos + "," + refPos + ")");
...
//
//  private boolean indelDisallowed(int readPos, int readLength, int mustIndel) {
//    if (!mFixedStart && !mFixedEnd) { //only check this indel if gotoh is perfoming the full alignment (this means seeded aligner currently can't benefit from this)
//      if (mustIndel == 0) { //first indel of a run
//        mGapOpens++;
//        if (mGapOpens > MAX_GAPOPENS_PER_ALIGNMENT) {
//          return true;
//        }
//      }
//    }
